 ...
 */
interface IPythPriceOracleGetter2 {
  /**
   ...
   */
  function getAssetPrice(address asset, bytes[] memory pythUpdateData) external returns (uint);
}
 ...
 */
interface IPythPriceOracle2 is IPythPriceOracleGetter2 {
  /**
   ...
   */
  function getAssetsPrices(address[] calldata assets, bytes[] memory pythUpdateData) external returns (uint[] memory);
